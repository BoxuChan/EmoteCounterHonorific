...
using EmoteCounterHonorific.Shared;
using Scriban;
using System;
...
public class ConfigWindow : Window
{
    ...
    private EmoteCounterSynchronizer EmoteCounterSynchronizer { get; init; }
    ...
    public ConfigWindow(Config config, ExcelSheet<Emote> emoteSheet, EmoteCounterSynchronizer emoteCounterSynchronizer, IPlayerState playerState, IDalamudPluginInterface pluginInterface, IPluginLog pluginLog) : base("EmoteCounterHonorific - Config##configWindow")
    {
        SizeConstraints = new()
        {
        ...
        EmoteCounterSynchronizer = emoteCounterSynchronizer;
    ...
    public override void Draw()
    {
        ...
        {
            if(EmoteCounterSynchronizer.TryUpdate(Config))
            {
                SaveConfig();
                PluginLog.Info("Successfully synced with emotecounter");
            } 
            else
            {
                PluginLog.Error("Failed to sync with emotecounter");
            }
        }
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Overrides internal counters with EmoteCounter ones\nCounters can be displayed using: /echonorific info");
        }
